...
```
```要删除 `_posts`、`_drafts`、`_wiki`、images文件夹下除了 template.md 以外的所有文件，然后添加你自己的内容，该删除的删除，该修改的修改全部完成之后，上传所有的代码，如何上传代码上面有讲，至此所有配置完成了。这个个人微博搭建是在别人的模板基础上进行的，没有去看 Jekyll 的配置。你们有兴趣可以自己学习，傻瓜教程。
